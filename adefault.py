#@title DEFAULT - Non fornisce tutti i campi del json
...
def predicted_action_to_json_action(predicted_agent_frame_action, tick, prev_mouse_buttons, prev_keyboard_keys):
    ...
    json_output = {
        "mouse": {
            "x": 640.0, "y": 360.0,  # Default/fixed values from example
            "dx": 0.0, "dy": 0.0,
        ...
        "hotbar": 0, # Default
        "tick": tick,
        "isGuiOpen": False # Default
    }

    # Mouse camera
    # predicted_agent_frame_action["camera"] is [pitch_change, yaw_change]
    # JSONL dx/dy are raw mouse movements before MineRL scaling
    if CAMERA_SCALER != 0:
    ...
    else: # Avoid division by zero, though CAMERA_SCALER is a constant here
    ...
    json_output["mouse"]["buttons"] = sorted(list(set(current_mouse_buttons))) # Ensure unique and sorted
    ...
    # Hotbar - assumes pressing hotbar.N key changes the selected slot to N-1
    # If multiple hotbar keys are predicted, takes the smallest index.
    # Defaults to 0 if no hotbar key is pressed.
    selected_hotbar_slot = -1 # Use -1 to indicate no selection yet from prediction
    for i in range(1, 10): # hotbar.1 to hotbar.9
    ...
    # If no hotbar key was pressed, it defaults to 0 (which might be from the previous state,
    # but this simplified version just sets it if a key is pressed, otherwise it's the initial 0).
    # For a true stateful hotbar, one would need to track the current hotbar slot across frames.
    # The provided JSONL example has 'hotbar: 0' consistently, implying it either doesn't change
    # or is logged as the active slot. This implementation sets it if a hotbar key is predicted.
